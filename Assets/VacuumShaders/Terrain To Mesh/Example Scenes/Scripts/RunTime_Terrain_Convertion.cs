...
{
    public enum RENDER_PIPELINE { Standard, Lightweight};
    //////////////////////////////////////////////////////////////////////////////
    //                                                                          // 
    //Variables                                                                 //                
    //                                                                          //               
    ...
    Material GenerateMaterial_Splatmap()
    {
        ...
        else if (usedTexturesCount == 1)
        {
            ...
            Shader shader = Shader.Find(renderPipeline == RENDER_PIPELINE.Lightweight ? "LightweightPipeline/Standard (Physically Based)" : "Legacy Shaders/Diffuse");
            if (shader != null)
            {
        ...
        string shaderName;
        if (renderPipeline == RENDER_PIPELINE.Lightweight)
            shaderName = string.Format("VacuumShaders/Terrain To Mesh/LightweightPipeline/Standard (Physically Based)" + "/{0} Textures", usedTexturesCount);
        else
            shaderName = string.Format("VacuumShaders/Terrain To Mesh/Standard/" + (canBeUsedBump ? "Bumped" : "Diffuse") + "/{0} Textures", usedTexturesCount);
